...
import com.example.codal.databinding.ActivityCodeForcesListBinding
import com.example.codal.databinding.ActivityPlatformQuestionsBinding
...
class PlatformQuestions : AppCompatActivity(),adapterOnClick {
    lateinit var binding: ActivityPlatformQuestionsBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityPlatformQuestionsBinding.inflate(layoutInflater)
        setContentView(binding.root)
        supportActionBar?.hide()
        db = Firebase.firestore
        collect = db.collection("userMods")
        collect
            .orderBy("ques", Query.Direction.DESCENDING)
            .get()
